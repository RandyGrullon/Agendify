...
import { Shield, Lock, Fingerprint, Eye, EyeOff, AlertCircle, CheckCircle } from "lucide-react";
...
export default function SecuritySettings() {
    ...
    const handleTogglePin = () => {
        if (pinEnabled) {
            // Disable PIN
            if (confirm("¿Estás seguro de desactivar el PIN? Esto removerá la protección de tu app.")) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className={`p-6 rounded-lg border-2 ${pinEnabled ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
                <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-full ${pinEnabled ? 'bg-green-100' : 'bg-yellow-100'}`}>
                        {pinEnabled ? (
                            <CheckCircle className={`w-6 h-6 ${pinEnabled ? 'text-green-600' : 'text-yellow-600'}`} />
                        ) : (
                            <AlertCircle className="w-6 h-6 text-yellow-600" />
                        )}
                    </div>
                    <div className="flex-1">
                        <h3 className={`text-lg font-semibold mb-1 ${pinEnabled ? 'text-green-900' : 'text-yellow-900'}`}>
                            {pinEnabled ? "Tu app está protegida" : "Tu app no está protegida"}
                        </h3>
                        <p className={`text-sm ${pinEnabled ? 'text-green-700' : 'text-yellow-700'}`}>
                            {pinEnabled 
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">Protege tu app con un código PIN</p>
                            </div>
                        </div>
                    ...
                    <div className="p-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="font-semibold text-gray-900">Autenticación Biométrica</h4>
                                    <p className="text-sm text-gray-600">Face ID, Touch ID o huella digital</p>
                                </div>
                            </div>
                            <label className="relative inline-flex items-center cursor-pointer">
                                <input
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <div className="p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h4 className="font-semibold text-gray-900">Bloqueo Automático</h4>
                                <p className="text-sm text-gray-600">Tiempo de inactividad antes de bloquear</p>
                            </div>
                        </div>
                        <select
                            value={autoLockTime}
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className="p-6">
                    <h4 className="font-semibold text-gray-900 mb-2">Tutorial de la Aplicación</h4>
                    <p className="text-sm text-gray-600 mb-4">
                        Reinicia el tutorial para verlo de nuevo la próxima vez que abras la app
                    </p>
                    <button

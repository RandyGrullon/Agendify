...
interface ImportDialogProps {
  ...
  onClose: () => void;
  onImport: (
    items: Omit<AgendaItem, "id" | "userId" | "createdAt" | "updatedAt">[]
  ) => Promise<void>;
}
...
export default function ImportDialog({
  ...
  const validateRow = (
    ...
    if (!row["FECHA"] && !row.fecha && !row.date) {
      ...
      data.date =
        typeof rawDate === "number" ? (rawDate as number) : String(rawDate);
    }
    ...
    if (!row["TIPO DE SERVICIO"] && !row.servicio && !row.service) {
      ...
      data.service = String(
        row["TIPO DE SERVICIO"] ?? row.servicio ?? row.service
      );
    }

    // Optional fields
    data.location = String(row["LUGAR"] ?? row.ubicacion ?? row.location ?? "");
    data.peopleCount = parseInt(
      String(row["CANT. PERSONAS"] || row.personas || row.peopleCount || "1")
    );
    data.quotedAmount = parseFloat(
      String(row["MONTO COTIZACION"] || row.monto || row.quotedAmount || "0")
    );
    data.comments = String(
      row["COMENTARIOS"] ?? row.comentarios ?? row.comments ?? ""
    );
    ...
    data.collaboratorPayment = parseFloat(
      String(row.pagoColaborador || row.collaboratorPayment || "0")
    );

    const status = String(
      row["STATUS"] ?? row.estatus ?? row.status ?? "pending"
    ).toLowerCase();
  ...
  const parseFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsed = (jsonData as unknown[]).map((row, index) =>
          validateRow(row as Record<string, unknown>, index + 1)
        );
        ...
        if (validCount === 0) {
          ...
          toast.success(
            `${validCount} filas listas para importar${

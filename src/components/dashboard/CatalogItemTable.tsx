...
export default function CatalogItemTable({ items, onEdit }: CatalogItemTableProps) {
    const { user } = useAuth();
    const router = useRouter();
    const [searchTerm, setSearchTerm] = useState('');
    const [filterType, setFilterType] = useState<'all' | 'storable' | 'digital' | 'service'>('all');
    ...
    const getTypeBadge = (type: string) => {
        ...
        return (
            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${badges[type as keyof typeof badges]}`}>
                {getTypeIcon(type)}
                ...
                <div className="bg-white rounded-lg shadow p-4 space-y-3">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2">
                        {['all', 'storable', 'digital', 'service'].map((type) => (
                            ...
                            >
                                {type === 'all' ? 'Todos' :
                                    type === 'storable' ? 'Almacenables' :
                                        type === 'digital' ? 'Digitales' : 'Servicios'}
                            </button>
                        ))}
                    </div>
                </div>
                            ...
                            <div className="px-4 py-3">
                                ...
                                {item.type === 'storable' && item.sku && (
                                    <div className="mt-3 pt-3 border-t border-gray-200">
                                        <p className="text-xs text-gray-500">SKU: <span className="font-mono text-gray-900">{item.sku}</span></p>
                                    </div>
                                )}
                            </div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {item.type === 'storable' && item.stock !== undefined && (
                                            <div className="text-sm text-gray-900">
                                                Stock: <span className="font-semibold">{item.stock}</span> {item.unit || 'u'}
                                            </div>
                                        )}
                                        {item.type === 'digital' && item.format && (
                                            <div className="text-sm text-gray-900">

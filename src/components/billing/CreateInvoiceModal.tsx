...
import { Client, AgendaItem, InvoiceItem } from "@/types";
...
export default function CreateInvoiceModal({ isOpen, onClose }: CreateInvoiceModalProps) {
    ...
    const clientAppointments = appointments.filter(
        app => selectedClient && (app.clientId === selectedClient.id || app.client === selectedClient.name)
    );
    ...
    const handleCreateInvoice = async () => {
        ...
        try {
            const selectedApps = clientAppointments.filter(app => selectedAppointments.includes(app.id));
            
            const items: InvoiceItem[] = selectedApps.map(app => ({
                description: `${app.service} - ${format(new Date(app.date), "d MMM", { locale: es })}`,
                quantity: 1,
                                ...
                                <div className="sm:flex sm:items-start">
                                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                                        <Dialog.Title as="h3" className="text-xl font-bold leading-6 text-gray-900 mb-6">
                                            Nueva Factura {step === 1 ? "- Seleccionar Cliente" : "- Seleccionar Citas"}
                                        </Dialog.Title>
                                                    ...
                                                    {({ open }) => (
                                                        <div className="relative mt-1">
                                                            <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left border border-gray-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 sm:text-sm">
                                                                ...
                                                                />
                                                                <Combobox.Button ref={clientButtonRef} className="absolute inset-y-0 right-0 flex items-center pr-2">
                                        ...
                                        {step === 2 && (
                                            <div className="mt-4">
                                                <p className="text-sm text-gray-500 mb-4">
                                                    Selecciona las citas que deseas incluir en esta factura para <strong>{selectedClient?.name}</strong>.
                                                </p>
                                                
                                                {clientAppointments.length === 0 ? (
                                                    <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                                        <Calendar className="mx-auto h-8 w-8 text-gray-400" />
                                                        <p className="mt-2 text-sm text-gray-500">No hay citas registradas para este cliente.</p>
                                                    </div>
                                                ) : (
                                                    <div className="max-h-60 overflow-y-auto border rounded-md divide-y divide-gray-200">
                                                        {clientAppointments.map((app) => (
                                                            <div 
                                                                key={app.id} 
                                                                className={`p-3 flex items-center justify-between cursor-pointer hover:bg-gray-50 ${selectedAppointments.includes(app.id) ? 'bg-blue-50' : ''}`}
                                                                onClick={() => toggleAppointment(app.id)}
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    <div className={`w-5 h-5 rounded border flex items-center justify-center ${selectedAppointments.includes(app.id) ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
                                                                        {selectedAppointments.includes(app.id) && <Check className="w-3 h-3 text-white" />}
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-sm font-medium text-gray-900">{app.service}</p>
                                                                        <p className="text-xs text-gray-500">{format(new Date(app.date), "d MMM yyyy", { locale: es })} - {app.time}</p>
                                                                    </div>
                                                                </div>
                                                                <span className="text-sm font-semibold text-gray-900">${app.quotedAmount}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}

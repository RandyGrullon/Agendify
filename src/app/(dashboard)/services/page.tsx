...
export default function CatalogPage() {
    ...
    const [enabledTypes, setEnabledTypes] = useState<CatalogItemType[]>(['service']);
    ...
    const storableCount = catalogItems.filter(item => item.type === 'storable').length;
    const digitalCount = catalogItems.filter(item => item.type === 'digital').length;
    ...
    return (
        <div className="p-4 sm:p-0 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Catálogo de Ítems</h1>
                    <p className="text-sm sm:text-base text-gray-600">Gestiona tus productos, servicios y contenido digital</p>
                </div>
                <button
                ...
                {enabledTypes.includes('storable') && (
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 border border-green-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm text-green-700 font-medium">Almacenables</p>
                                <p className="text-2xl font-bold text-green-900">{storableCount}</p>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4 border border-purple-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-purple-900">{digitalCount}</p>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-blue-900">{serviceCount}</p>
                            </div>
                        </div>
                    </div>
                )}
